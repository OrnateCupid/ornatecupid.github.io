...
1. 释放内存：必须在请求结束时释放内存。springboot 使用线程池，每个请求使用线程池中的一个线程，新请求可能会读到应当销毁的数据造成内存泄漏。

2. key 的弱引用：保证当 key 丢失时（一个 threadLocal 实例的引用丢失），GC 可以回收 threadLocal 本身。但 value 是强引用，没法回收
